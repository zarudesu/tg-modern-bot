...
# Обработчики текстовых сообщений для кастомного ввода
@router.message(F.text, F.func(lambda msg: msg.text and not msg.text.startswith("/")))
async def handle_text_input(message: Message):
    """Обработчик текстовых сообщений для кастомного ввода"""
    try:
        user_id = message.from_user.id
        ...
        async for session in get_async_session():
            ...
            if not user_state:
                return  # Игнорируем сообщения без активного состояния
            ...
            elif user_state.current_state == WorkJournalState.ENTERING_CUSTOM_COMPANY.value:
                await handle_custom_company_input(message, session, service, text)
            
            elif user_state.current_state == WorkJournalState.ENTERING_CUSTOM_DURATION.value:
            ...
            elif user_state.current_state == WorkJournalState.ENTERING_CUSTOM_WORKER.value:
...
async def handle_custom_worker_input(
    ...
    user_state = await service.get_user_state(message.from_user.id)
    ...
    await show_confirmation(message, session, service, user_state)
